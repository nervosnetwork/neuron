describe(`Should subscribe to 2 subjects on importing`, () => {
  ...
  jest.doMock('models/subjects/address-created-subject', () => ({ getSubject: () => ({ subscribe: stubbedAddressCreatedSubjectSubscribe }) }))
  jest.doMock('models/subjects/wallet-deleted-subject', () => ({ getSubject: () => ({ subscribe: stubbedWalletDeletedSubjectSubscribe }) }))
